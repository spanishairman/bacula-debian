...
Сетевые взаимодействия между компонентами _Bacula_. Для корректной работы, нам потребуется открыть некоторые порты на межсетевом экране, если такой имеется в нашей сети. Следующий список поможет настроить правила файервола:
```
