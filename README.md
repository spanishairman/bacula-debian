...
```
Здесь:
  - **File Retention = time-period-specification** -  Запись хранения файлов определяет продолжительность времени, в течение которого Bacula будет хранить записи файлов в базе данных каталога.  По истечении этого периода времени и если для параметра AutoPrune установлено значение yes , Bacula обрежет (удалит) записи файлов, которые старше указанного периода хранения файлов.  Удаление произойдет в конце задания резервного копирования для данного Клиента. Обратите внимание, что запись базы данных клиента содержит копию периодов хранения файлов и заданий,  но Bacula использует текущие значения, найденные в ресурсе клиента директора, для выполнения сокращения. Поскольку записи файлов в базе данных составляют, вероятно, 80 процентов размера базы данных, вам следует тщательно определить, какой именно период хранения файлов вам нужен.  После удаления записей файлов из базы данных вы больше не сможете восстанавливать отдельные файлы в задании.  Однако в Bacula версии 1.37 и более поздних версиях, пока запись задания существует, вы сможете восстановить все файлы задания. Периоды хранения указываются в секундах, но для удобства существует ряд модификаторов, которые позволяют легко указать минуты, часы, дни, недели, месяцы, кварталы или годы записи.  Дополнительные сведения о спецификации модификатора см. в главе «Конфигурация» «Время» данного руководства. Срок хранения файлов по умолчанию составляет 60 дней. 
  - **Job Retention = time-period-specification** -  Запись о хранении заданий определяет продолжительность времени, в течение которого Bacula будет хранить записи о заданиях в базе данных каталога.  По истечении этого периода времени и если для параметра AutoPrune установлено значение «да» , Bacula обрежет (удалит) записи заданий, которые старше указанного периода хранения заданий.  Обратите внимание: если запись задания выбрана для сокращения, все связанные File and JobMedia records также будут удалены независимо от установленного периода хранения файлов.  Как следствие, вы обычно устанавливаете период хранения файлов меньше, чем период хранения задания. Как упоминалось выше, после удаления записей файлов из базы данных вы больше не сможете восстановить отдельные файлы из задания.  Однако пока запись задания остается в базе данных, вы сможете восстановить все файлы, резервные копии которых были созданы для задания (в версии 1.37 и более поздних версиях).  Как следствие, обычно рекомендуется хранить записи заданий гораздо дольше, чем записи файлов. Период хранения указывается в секундах, но для удобства существует ряд модификаторов, которые позволяют легко указать минуты, часы, дни, недели, месяцы, кварталы или годы.  Дополнительные сведения о спецификации модификатора см. в главе «Конфигурация» «Время» данного руководства. Период хранения заданий по умолчанию составляет 180 дней.
  - ** AutoPrune = yes/no** -  Если для параметра AutoPrune установлено значение «да» (по умолчанию), Bacula автоматически применит период хранения файла и срок хранения задания для Клиента в конце задания. Если вы отключите это, установив для него значение no , ваш Каталог будет расти каждый раз, когда вы запускаете задание.
